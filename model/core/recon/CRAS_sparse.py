...
def kras_cras_sparse_pdata_bounds(
    ...
    for it in tqdm(range(1, max_iter + 1), desc="KRAS-CRAS iterations"):
        ...
        for i in row_order:
            ...
            r_cand = float(np.clip(r_cand, 1e-12, 1e12))
            ...
            def interval_for_pos(a_j, l_j, u_j):
            ...
            def interval_for_neg(a_j, l_j, u_j):
            ...
            if np.any(pos_mask):
                js_pos = js[pos_mask]
                for j_idx in js_pos:
                    rlo_j, rhi_j = interval_for_pos(a[j_idx], l[j_idx], u[j_idx])
            ...
            if r_lower <= r_upper and np.any(neg_mask):
                js_neg = js[neg_mask]
                for j_idx in js_neg:
                    rlo_j, rhi_j = interval_for_neg(a[j_idx], l[j_idx], u[j_idx])
                    r_lower = max(r_lower, rlo_j)
                    r_upper = min(r_upper, rhi_j)
                    if r_lower > r_upper:
                        break
            ...
            r_lower = max(r_lower, 1e-12)
            r_upper = min(r_upper, 1e12)
            ...
            r_safe = float(np.clip(r_safe, 1e-12, 1e12))
...
def init_problem(
        G_path = r"data/proc/const/mb/G_mass_balance_20250930_112923.npz",
        c_path = r"data/proc/const/mb/c_mass_balance_20250930_112923.npy",
        sigma_path = r"data/proc/const/mb/c_sigma_mass_balance_20250930_112923.npy",
        c_idx = r"data/proc/const/mb/c_mass_balance_idx_20250930_112923.npy",
        a0_path = r"data/proc/ie/ie_20250930_104042.npy"):
    

    # --- Load data ---
    G = sparse.load_npz(G_path, )  # load sparse matrix
    c = np.load(c_path)     # load constraint targets
    sigma = np.load(sigma_path)  # load constraint tolerances
    
    a0 = np.load(a0_path)     # load initial guess

    # load constraint index map
    c_idx_map = np.load(c_idx, allow_pickle=True).item()   
    # switch keys and values
    c_idx_map = {v: k for k, v in c_idx_map.items()}

    return G, c, sigma, a0, c_idx_map
